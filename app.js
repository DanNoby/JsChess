...
function checkIfValid(target) {
  ...
  switch (piece) {
    case 'pawn':
      const starterRow = [8, 9, 10, 11, 12, 13, 14, 15]; // where black pawns start
      if (
        (starterRow.includes(startId) && startId + width * 2 === targetId) || // two steps forward from start
        startId + width === targetId || // one step forward
        (startId + width - 1 === targetId && document.querySelector(`[square-id="$"]`)) || // capture diagonally left
        (startId + width + 1 === targetId && target.firstChild) // capture diagonally right
      ) {
        return true;
      }
      break;
